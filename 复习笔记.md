...
2.为什么要调优
    减少GC频率，减少FullGC的次数，以较小的内存实现低响应时间高吞吐量
● 防止出现OOM
● 解决OOM
● 减少Full GC出现的频率
...
《[JUC线程池ThreadPoolExecutor源码分析](http://www.throwable.club/2019/07/15/java-concurrency-thread-pool-executor/)》

[Java线程池实现原理及其在美团业务中的实践 (qq.com)](https://mp.weixin.qq.com/s?__biz=MjM5NjQ5MTI5OA==&mid=2651751537&idx=1&sn=c50a434302cc06797828782970da190e&scene=21#wechat_redirect)

```java
// execute 和 submit 的执行流程
```

```java
// 使用线程池的原因
	降低资源消耗：重复利用已创建的线程，减少线程创建和销毁带来的消耗
	提高响应速度：任务到达时，可以不创建线程就执行任务
    提高线程的可管理性：统一管理，防止线程过多88系统资源耗尽
...
// 流程
    // 大部分会执行第 (2) 步 
    (1) 如果运行线程少于corePoolSize，就会创建新的线程去执行任务
    (2) 如果运行线程大于等于corePoolSize，就会把任务放入工作队列
    (3) 如果无法放入工作队列，就会创建新的线程处理任务
    (4) 如果创建新线程使得当前线程数超过maximumPoolSize，就会执行拒绝策略
// 创建线程池的参数
    ...
    (7) RejectedExecutionHandler // 拒绝策略，当提交的任务过多而不能及时处理时，我们可以定制策略来处理任务;
    	拒绝策略有几种：
    	1) AbortPolicy ：抛出异常来拒绝新任务的处理
    	2) DiscardPolicy ：不处理新任务，直接丢弃掉
    	3) DiscardOldestPolicy ：将丢弃最早的未处理的任务请求
    	4) CallerRunsPolicy：调用执行自己的线程运行任务，也就是直接在调用execute() 方法的线程中运行被拒绝的任务，如果执行程序已关闭，则会丢弃该任务。
    
// 线程池的五种状态
    running、shutdown、stop、tidying、terminate
    关闭线程池的原理：遍历线程池中的所有线程，逐个调用线程的interrupt()方法，如果线程不能响应中断的任务，可能永远无法停止。
    shutdown()只是将线程池设置为shutdown状态，中断所有没有执行任务的工作线程，工作队列中的任务会执行完。
    shutdownNow()会将线程池设置为stop状态，停止所有工作线程，并清空工作队列，返回等待执行的任务列表。
    
// 如何配置线程池的参数
    如果我们设置的线程池数量太小的话，如果同一时间有大量任务/请求需要处理，可能会导致大量的请求/任务在任务队列中排队等待执行，甚至会出现任务队列满了之后任务/请求无法处理的情况，或者大量任务堆积在任务队列导致 OOM。这样很明显是有问题的！ CPU 根本没有得到充分利用。
	但是，如果我们设置线程数量太大，大量线程可能会同时在争取 CPU 资源，这样会导致大量的上下文切换，从而增加线程的执行时间，影响了整体执行效率。
    // 选取方法
    对于CPU密集型任务，线程池的个数少一些，因为大部分的任务处于一直占用CPU的状态，所以呢大于CPU个数一点就好了；对于IO密集型任务呢，大部分处于IO等待，所以CPU空闲，线程池个数多就可以在空闲期去执行其他的任务。
    
// Executor的成员结构
    Runnable和Callable的对比
		Runnable接口不会返回结果或抛出检查异常，但是Callable接口可以。所以，如果任务不需要返回结果或抛出异常推荐使用Runnable接口，这样代码看起来会更加简洁。
    	工具类 Executors可以实现将Runnable对象转换成Callable对象。(Executors.callable(Runnable task))
...
缺点：(1) maximumPoolSize和keepAliveTime是无效参数
      (2) 使用无界队列不会拒绝任务，可能会导致OOM
// CachedThreadPool
      /**
...
缺点：CachedThreadPool的corePoolSize被设置为0，maximumPoolSize被设置为 Integer.MAX.VALUE，即它是无界的，这也就意味着如果主线程提交任务的速度高于maximumPool中线程处理任务的速度时，CachedThreadPool会不断创建新的线程。极端情况下，这样会导致耗尽 cpu 和内存资源。
```
...
连接器是账号权限校验 分析器是库权限验证 执行器是语句在表中权限验证 我的理解是这样的
...
// 建表时存在的索引类型
    ...
    注：由于innodb数据和索引都在一个文件中，所以主键索引叶子节点存的是行记录，辅助索引叶子节点存的是主键值，需要回表；而myisam索引和数据文件分开，主键索引和辅助索引的叶子节点存的都是行记录的地址。
// 辅助索引
...
// 覆盖索引
    在索引结构的叶子节点中直接有查询的数据，不需要回表。
        
// 慢sql的建议
	(1) 使用连接代替子查询
	对于数据库来说，在绝大部分情况下，连接会比子查询更快，使用连接的方式，MySQL优化器一般可以生成更佳的执行计划，更高效地处理查询而子查询往往需要运行重复的查询，子查询生成的临时表上也没有索引， 因此效率会更低。
	...
	(5) 使用联合索引，而非建立多个单独索引，遵循最左匹配原则    
        
// 索引下推原则
    举例： select * from tuser where name like '张%' and age=10 and ismale=1;
	// 该表有(name,age)的联合索引，先找到'张%'的索引叶子节点，然后向后顺序遍历，判断该联合索引中的age字段的值和查询条件是否相同，这就是索引下推原则。
    针对联合索引的查询，MySQL 5.6 引入的索引下推优化（index condition pushdown)， 可以在索引遍历过程中，对索引中包含的字段先做判断，直接过滤掉不满足条件的记录，减少回表次数。
	在 MySQL 5.6 之前，只能一个个回表。到主键索引上找出数据行，再对比字段值。
        
// 普通索引和唯一索引怎么选
    // 主要考虑的是change buffer，changebuffer只有在内存中不存在更新数据的数据页时才会起作用，对于唯一索引，更新时会先将数据页加入bufferpool中进行判断，所以changebuffer不起作用
    这两类索引在查询能力上是没差别的，主要考虑的是对更新性能的影响。所以，我建议你尽量选择普通索引。如果所有的更新后面，都马上伴随着对这个记录的查询，那么你应该关闭 change buffer，因为这个查询会导致一个merge过程，就会从磁盘页加载数据页并更新数据，相当于增加了维护change buffer的过程。而在其他情况下，change buffer 都能提升更新性能。
    !!!!!!!redo log 主要节省的是随机写磁盘的 IO 消耗（转成顺序写），而 change buffer 主要节省的则是随机读磁盘的 IO 消耗。
        
// mysql优化器选择索引的策略 ？？？第十讲
    优化器选择索引的因素：扫描行数、是否using filesort、是否有临时表
    优化器选择索引和扫描行数有关，扫描行数和基数有关，也就是区分度，基数是抽样统计的
...
// 排序的种类
    sorted buffer
    全字段排序和rowid排序
    如果sorted buffer过小，就会在磁盘上创建临时表进行归并排序，内存中使用的是快速排序。
        
// mysql中索引失效的例子
    
    (1) select * from tradelog where id + 1 = 10000
        select count(*) from tradelog where month(t_modified)=7;
    (2) select * from tradelog where tradeid=110717; // 这里的tradeid是varchar类型，MySQL 中，字符串和数字做比较的话，是将字符串转换成数字，对于‘a’这种无法转化成具体数字的，会将其转化成0；这里相当于对索引多了一个转化的函数，所以无法使用索引了
    (3) 由于表的字符集不同，导致对索引有隐式的函数字符集转化
        
	总结：对索引字段做函数操作，可能会破坏索引值的有序性，因此优化器就决定放弃走树搜索功能。第二个例子是隐式类型转换，第三个例子是隐式字符编码转换，它们都跟第一个例子一样，因为要求在索引字段上做函数操作而导致了全索引扫描
```
...
2.四种隔离级别：读未提交，读已提交，可重复读，序列化
    读已提交和可重复读依照MVCC下的不同策略完成一致性读；“读未提交”隔离级别下直接返回记录上的最新值，没有视图概念；而“串行化”隔离级别下直接用加锁的方式来避免并行访问。
			
3.MVCC底层原理
    ！！！InnoDB 的行数据有多个版本，每个数据版本有自己的 row trx_id，每个事务或者语句有自己的一致性视图。普通查询语句是一致性读，一致性读会根据 row trx_id 和一致性视图确定数据版本的可见性。
    	// read-view的创建时机：第一种begin / start transaction启动，是在执行第一个select语句时创建的；第二种，是在执行 start transaction with consistent snapshot 时创建的。
		Ⅰ注：begin不是一个事务的起点，在执行到第一个操纵innodb表的语句时（增删改，读的事务id不一样），事务才算启动并向mysql申请事务id，并严格按照事务的启动顺序分配事务id
		Ⅱmysql数据库表会为数据库表多两列--事务id和回滚指针
    	// 除了一致性读的概念，还有当前读的概念，也就是说在修改表数据的语句时，不能依照一致性读读历史数据，只能读当前最新的版本数据，并进行修改
    ...
    // 总结      
    1.不管哪个隔离级别，写操作都是当前读，没有read view.
	2.RC级别下，事务中的每一条select语句都会创建一个read view，因此可能出现同一个事物中连续的两个select读到不一样的情况。
	3.RR级别下，start transaction with consistent snapshot会立刻生成一致性快照。start transaction启动事务会在第一次读操作的时候生成一致性快照。RR模式下，一次事务只会产生一个一致性快照。
	4. 快照隔离读有一个特例，能读到自己最新的更新，哪怕没提交。
	5. RC级别下即使使用start transaction with consistent snapshot也不会生效
...
// 单机配置优化
max_connection = 1000
innodb_file_per_table = 1 // 指的是表的索引和数据都放在独立的表空间中，即一个文件内，而如果设置为0的话，所有的表索引和数据都放在一整个共享表文件内，如果有增删改，则会十分影响性能。
innodb_buffer_pool_size = 1G
innodb_log_file_size = 256M
innodb_log_buffer_size = 16M
innodb_flush_log_at_trx_commit = 0\1\2 // redolog的刷盘时间    
```
...
2.为什么建议InnoDB表必须建主键，并且推荐使用整型并且自增的主键？
	...
	整型：查找时候做比较很快，而且在辅助索引的叶子节点中占用空间小。
	自增：插入节点时，在某一叶子节点的后边直接加入即可，方便索引排序。而使用uuid这种的话，可能会导致叶子节点分裂，会影响性能。// 防止插入数据的时候发生页分裂
    适合用业务字段直接做主键的场景：还是有的。比如，只有一个索引；该索引必须是唯一索引。这样该字段做主键就不会影响到其他的索引树了，因为只有一个索引啊！
...
// 关于MESI协议
影响MESI的时间瓶颈：
    // 说白了如果没有任何优化，需要无效化指令和确认响应完成了，才可以将变量写入CPU缓存中。有了store buffer，当缓存行是M或者E的时候可以直接写入缓存行，如果是S，则将数据和地址写入store buffer，再发送无效化指令，收到无效化确认才可以将store buffer中的数据和地址刷到缓存中。
    
    // 针对无效化指令的优化：引入Store Buffer，核心可以先将变量写入Store Buffer，便可以去执行其他指令，如果后面的操作需要用到这个变量，就可以从Store Buffer中读取变量的值，核心读数据的順序变成Store Buffer → 缓存 → 内存。当收到其他全部核心的无效化确认后就可以把store buffer的数据写入缓存中。 store buffer的引入使得CPU在执行写操作的时候可以不等待无效化确认，减少了写操作的延时，使得在其他CPU回复无效化确认的 时间段内能够执行其他指令。
	(1) 无效化指令：Kernel0需要通知所有的核心该变量对应的缓存在其他核心中是无效的。在通知完之前，该核心不能做任何关于这个变量的操作。
    // 针对确认响应的优化：引入Invalidate Queue这个结构。其他核心收到Kernel0的Invalidate的命令后，立即给Kernel0回Acknowledge，并把Invalidate这个操作，先记录到Invalidate Queue里，当其他操作结束时，再从Invalidate Queue中取命令，进行Invalidate操作。所以当Kernel0收到确认响应时，其他核心对应的缓存行可能还没完全置为Invalid状态。           处理器在接收到Invalidate 消息之后井不删除消息中指定地址对应的副本数据，而是将消息存入无效化队列之后就回复Invalidate Acknowledge 消息，从而减少了写操作执行处理器所需的等待时间。有些处理器（比如x86)可能没有使用无效化队列。
	(2) 确认响应：Kernel0需要收到其他核心的确认响应。在收到确认消息之前，该核心不能做任何关于这个变量的操作，需要持续等待其他核心的响应，直到所有核心响应完成，将其对应的缓存行标志位设为Invalid，才能继续其它操作。
    
    // Store Buffer和Invalidate Queue的优化会导致一致性的问题：store buffer会导致storeload重排序
    针对Store Buffer：核心在后续变量的新值写入之前，把Store Buffer的所有值刷新到缓存；核心要么就等待刷新完成后写入，要么就把后续的后续变量的新值放到Store Buffer中，直到Store Buffer的数据按顺序刷入缓存。这种也称为内存屏障中的写屏障（Store Barrier）。// 内存屏障中的存储屏障（ Store Barrier ）可以使执行该指令的处理器冲刷其写缓冲器。
	针对Invalidate Queue：执行后需等待Invalidate Queue完全应用到缓存后，后续的读操作才能继续执行，保证执行前后的读操作对其他CPU而言是顺序执行的。这种也称为内存屏障中的读屏障（Load Barrier）。// 内存屏障中的加载屏障(Load Barrier )正是用来解决这个问题的。加载屏障会根据无效化队列内容所指定的内存地址，将相应处理器上的高速缓存中相应的缓存条目的状态都标记为I ，从而使该处理器后续执行针对相应地址（无效化队列内容中指定的地址）的读内存操作时必须发送Read 消息，以将其他处理器对相关共享变量所做的更新同步到该处理器的高速缓存中。
```
...
5.线程
   ...
   (2) 实现方式：
        ...
        1) 用户线程(M:1)：由用户态的线程管理库管理，也就是在进程内部，内核访问不到，只能访问到该进程；进程中会维护TCB，TCB中记录了线程私有的栈、寄存器；用户线程切换无需切换到内核态
        缺点：Ⅰ一个用户线程阻塞会导致进程所包含的用户线程都不能执行，因为操作系统看不到线程，只能看到进程，线程阻塞相当于进程阻塞；Ⅱ用户线程运行后，除非主动交出使用权，否则其他用户线程无法打断当前运行线程，因为切换线程需要在内核态，操作系统才有。
        2) 内核线程(1:1)
        3) 混合模式(M:N)：用户态的执行系统负责进程内部线程在非阻塞时的切换；内核态的操作系统负责阻塞线程的切换，即我们同时实现内核态和用户态线程管理。其中内核态线程数量较少，而用户态线程数量多。每个内核态线程可以服务一个或多个用户态线程。换句话说，用户态线程被多路复用到内核态线程上。
          轻量级进程(LWP)：是内核支持的用户线程，一个进程可以有一个或多个LWP,一个LWP对应一个内核线程。简单理解，LWP代表的就是程序的执行线程，只不过可以由内核访问到。
    (3) 一个进程所能创建线程的最大数量：
    	1) 32 位系统，用户态的虚拟空间只有 3G，如果创建线程时分配的栈空间是 10M，那么一个进程最多只能创建 300 个左右的线程。
    	2) 64 位系统，用户态的虚拟空间大到有 128T，理论上不会受虚拟内存大小的限制，而会受系统的参数或性能限制。如：/proc/sys/kernel/threads-max，表示系统支持的最大线程数，默认值是 14553；
...
10.协程
    协程是基于线程的内存模型，即一个函数内部有多个返回点，并且在每个返回点返回的时候，进程内部会维护函数的相关信息，下次调用该协程的时候会继续从上一个返回点继续执行。普通函数只是协程的一种特例。协程是串行的方式运行，不需要加锁。
    线程的切换(调度)是由操作系统实现的，对程序⚪不可见，而协程是在用户态实现的，由程序员操控，所以切换比线程快很多。协程的切换只有内存开销，没有CPU的开销。遇到阻塞的时候就会归还执行权，用代码同步的方式模拟异步编程。
    总的来说协程对于操作系统不可见。它开辟在内存中的堆区，以应对在context切换时避免需要保存的栈帧来回复制的开销。
```
...
```java
目的：解决http的 ①窃听、②篡改(植入垃圾广告)、③冒充
     于是在http和tcp之间加入了TLS层：// TLS的握手阶段是在TCP握手之后，⽬的是为了通过⾮对称加密握⼿协商或者交换出对称加密密钥
    	(1) 信息加密：
    		// 混合加密：非对称和对称，即公钥+私钥
    		// 非对称加密：服务器公钥加密，服务器私钥解密。
    		// 对称加密：对称密钥加密，对称密钥解密
    		在加密应用信息的时候使用对称加密密钥，而对称加密密钥不能泄露，所以使用非对称加密的方式保护对称加密密钥的生成，这个过程就是密钥交换算法负责的。
    	(2) 校验机制：如果篡改内容就不会正常显示 // 摘要算法
    	(3) 数字证书(服务器信息+数字签名)：通过数字证书的方式保证服务器公钥的身份，解决被冒充的风险 

// TLS1.2 需要四次握手
    // RSA过程的问题：不支持前向加密，因为服务器的私钥可能被破解，所以pre-master可能被破解，也就是说会话密钥会被破解，导致之前传送的报文都会被破解。所以使用ECDHE握手，可以支持前向保密(原理就是双方都有自己的私钥)，和RSA不一样的是，TLS第二次握手中，服务端会发送Server Key Exchange，生成ECDHE的私钥和公钥等；TLS第三次握手会就可以发送加密数据，而RSA在完成四次握手之后才可以发送加密数据。
    // ECDHE加密过程：客户端随机数1 + 服务端随机数1 + x(ECDHE共享密钥),x = 客户端私钥(客户端随机数2) + 服务端公钥(或者客户端公钥 + 服务端私钥(服务端随机数2))
    // RSA握手过程：
    // 密码套件：密钥交换算法+签名算法+对称加密算法+摘要算法    
    (1) 第一次握手
    	客户端发送TLS版本号、客户端生成的随机数、支持的密码套件列表
    (2) 第二次握手
    	服务器支持的TLS版本号、服务器随机数、选择的密码套件、服务器的数字证书
    (3) 第三次握手
    	1) 首先客户端验证数字证书：客户端拿着CA的公钥解密证书中的数字签名得到hash1，接着对证书中服务器的信息采取签名算法得到hash2，如果hash1 = =hash2，表示数字证书可信，可以获取服务端的公钥。
    	2) 客户端生成随机数pre-master，用服务器RSA公钥加密发送给服务器，服务器用RSA私钥解密。此时客户端和服务器用Client Random、Server Random、pre-master生成会话(对称)密钥(Master Secret)
    	3) 客户端发送Change Cipher Spec，告诉服务端开始使用加密方式发送消息
    	4) 客户端发送Encrypted Handshake Message，把之前发送的数据做个摘要再用会话密钥加密，让服务器验证加密通信是否可用和握手信息是否又被篡改过。
    (4) 第四次握手
    	服务器发送Change Cipher Spec 和 Encrypted Handshake Message
    
https如何优化
    // TLS握手次数多、会话密钥加密
    (1) TLS升级：TLS1.3 需要两次握手
    (2) 证书优化：
    	1) 证书传输优化：使用ECDSA而不是RSA，因为在相同安全强度下，ECC的密钥长度会比RSA短
    	2) 证书验证优化：思路是优化如何知道证书的有效状态，由CRL->OCSP->OCSP Stapling(服务器开启此功能后，预先知道了证书的有效状态，把结果缓存起来，待握手时直接发送给客户端，客户端就不用再去请求CA验证证书的有效状态了)
    (3) 会话复用：
    	// 1) 2) 只需要一个RTT就可以回复会话
    	1) Session ID：服务器端和客户端都缓存第一次的会话密钥，客户端下一次访问的时候带着sessionid来找会话密钥
    	2) Session Ticket：服务器端不再缓存会话密钥，加密发送给客户端，再连接的时候以cookie方式发给服务器
    	3) TLS1.3使用Pre-shared Key，0 RTT就可以恢复会话。
```
...
相比http1.0的改进：
    (1) 使用长连接
    (2) 支持管道传输，即发送一个请求不必等待其响应就可以发送下一个请求
    
http1.1的性能问题：
    (1) 只能压缩body，header部分不能压缩，而且会发送冗余重复的header
    (2) 按顺序响应，无优先级控制 // 队头阻塞问题
    (3) 请求只能从客户端开始，服务器只能被动响应
    
http1.1如何优化
   (1) 避免发送http请求：
    	使用本地缓存避免发送http请求，url为key，响应体为value。客户端第一次响应后将其缓存在磁盘中，在响应头中带有过期时间，下一次请求时，如果缓存没过期，就直接读取本地缓存；如果过期，重新请求的时候在请求的头部带上第一次请求响应头部中的摘要，可以唯一标识响应的资源，如果服务器中资源没变，响应304(缓存重定向)即可。
    (2) 减少http请求次数：
    	1) 减少重定向的次数：针对代理服务器才可以这么做，正常具有代理服务器访问源服务器的话，产生重定向会有四次http请求，而第一次重定向在代理服务器完成而不用返回给客户端就可以 变为3次http，再有此客户端请求来时，代理服务器记住了直接"重定向",变为2次正常情况的两次http
    	2) 合并请求：把请求多个小文件的请求，变成一个请求，虽然请求资源一样，但减少了重复发送的请求头；// http1.1如果想要并发请求就需要建立不同的tcp连接，在一条tcp连接上就会有队头阻塞的问题，而且每条tcp连接都会有握手和慢启动的问题。
    (3) 减少http响应数据大小
    	无损压缩：文本、源代码
    	有损压缩：图片、视频、音频 
...
和http1.1相比：
    (1) 基于https
    // key由表中的索引号表示，value由huffman编码组成
    (2) 用HPACK算法压缩头部：在客户端和服务器同时维护一个头部信息表，所有字段存入并生成一个索引号，以后只发送索引号即可，头部信息表分为静态表和动态表;同时使用huffman编码压缩数据 // 解决http1.1 (1)
    (3) 不像http1.1使用文本报文，header和body采用二进制报文
    // http2是如何解决并发传输的没看明白，stream、message、frame？
    (4) 多路复用：不用按照顺序响应(如果发现某个请求耗时就会先响应该请求已经处理好的数据和后面的请求，等到处理完再响应该请求剩余的数据) // 解决http1.1 (2) 
    (5) 服务器推送 // 解决http1.1 (3)

http2的问题：
    多个http复用一个tcp连接，当发生丢包而超时重传的时候，所有的http请求都会阻塞等待重传。
```

##### 4.2.1.4 http3

```java
// http1.1队头阻塞和http2包重传的问题都是tcp的原因
```
...
```java
// 三次握手和四次挥手的过程状态等没总结0.0

// TCP 和 UDP 的区别，它们的头部结构是什么样的
...
// TCP 的 keepalive，以及和 HTTP 的 keepalive 的区别？
	(1) http的keepalive目的在于复用tcp连接，减少三次握手和四次挥手带来的消耗。有两个重要参数:
	1) keepalive_timeout：一个http产生的tcp连接在传送完最后一个响应后，还需要hold住keepalive_timeout秒后，才开始关闭这个http连接；      2)maxKeepAliveRequests：在经过该次数的http请求后，http连接会关闭。
    (2) TCP协议(四层)的KeepAlive机制意图在于保活、心跳，检测连接错误。有以下三个参数：
     1) tcp_keepalive_intvl：keepalive探测包的发送间隔，默认为75秒
	2) tcp_keepalive_probes：如果对方不予应答，探测包的最大发送次数，默认为9次。即连续9次发送，都没有应答的话，则关闭连接。
	3) tcp_keepalive_time：连接的最大空闲(idle)时间，默认为7200秒，即2个小时。需要注意的是，这2个小时，指的是只有keepalive探测包，如果期间存在其他数据传输，则重新计时。
...
```java
1.TCP是⾯向连接的、可靠的、基于字节流的传输控制协议

// 连接！！！
2.⽤于保证可靠性和流量控制所维护的某些状态信息的组合，包括Socket、序列号和窗⼝⼤⼩称为连接。// 三次握手后才可以初始化Socket、序列号和窗⼝⼤⼩来保证TCP连接说可靠的
    
3.MTU是网络传输最大报文包，MSS是网络传输数据最大值。
...
第一次握手失败：也就是SYN丢失，由tcp_syn_retries指定，Linux默认超时重传5次，每次RTO都会增大一倍。
第二次握手失败：当第⼆次握⼿的 SYN、ACK 丢包时，客户端会超时重发 SYN 包，服务端也会超时重传SYN、ACK 包，服务端的重传SYN、ACK包由tcp_synack_retries = 5 指定。
第三次握手失败：服务端会超时重传SYN、ACK 包，默认是5次，之后服务端断开连接，但客户端还处于连接状态。建立连接后的数据包最大超时重传次数由tcp_retries2 = 15 指定。如果超过了15次，tcp客户端就会断开连接；如果客户端不发送数据，tcp的保活机制起作用，在一个时间段内如果没有连接活动，就会每隔一个时间间隔发送探测报文，如果连续几个探测报文都没有响应，就会认为此tcp连接死亡。// 所谓的保活就是keepalive的相关机制
...
// 三次握手的异常情况
// 三次握手中半连接全连接队列溢出怎么办
    全连接溢出是因为客户端发送的连接太多了，当全连接队列满了，有两种策略，一种服务端直接丢弃客户端的ACK，一种服务端给客户端发送RST报文，销毁连接。大多数情况选择第一种，因为丢弃后，客户端还可以再次发送ACK和数据，等服务端错过高峰繁忙就可以处理连接了。
    半连接队列溢出一个场景就是客户端不回ACK，即SYN攻击。
...
(1) 三次握手优化 
    // 1是客户端，234是服务端
    1) 调整SYN报文的重传次数
    	根据网络的稳定性和目标服务器的重传次数，调整客户端三次握手的时间上限；比如适当降低SYN的重试次数
    2) 调整SYN半连接队列的长度
    3) 调整SYN+ACK报文的重传次数
    4) 调整全连接队列的长度
    5) 绕过三次握手 (tcp fast open)
    客户端优化：
    // 如何调整半连接队列大小：若增大半连接队列，不能只单纯增⼤ tcp_max_syn_backlog 的值，还需⼀同增⼤ somaxconn 和 backlog，也就是增⼤ accept 队列。否则，只单纯增⼤ tcp_max_syn_backlog 是⽆效的。最后还需要重启服务，因为SYN队列和accept队列都是在listen()中初始化的
(2) 四次挥手的优化
    // 主动方
    // 具体优化细节和原理不是很懂呢！！！
    (1) 调整FIN报文重传次数
    (2) 调整FIN_WAIT2报文重传次数 // close
    (3) 调整孤儿连接的个数 // close
    (4) 调整TIME_WAIT状态上限的个数
    (5) 复用TIME_WAIT状态的连接
```
...
// 破坏和丢包性质差不多，都会利用重传机制，丢包可能是超时或者快速，破坏也可能是超时或者快速，重复利用D-SACK算法，乱序是快速解决。
    (1) 超时重传：超过RTO的数据包没有收到ACK就会重传，两种情况：数据包丢了或者接收端发送的ACK丢了
    	问题：超时周期比较长
    // 快速重传只是丢失了个别的报文段，可以解决乱序问题
    ...
    (3) SACK：在TCP头部选项中加一个SACK的东西，用于接收端发送ACK时告诉发送端本次收到了哪些数据。// 如果不加就必须重传丢失包之后的每g
     // 当ACK大于SACK的序列号时，表示是D-SACK，数据包有重复发送，具体原因发送方可知
    (4) D-SACK：可以让发送方知道，是数据包丢了还是ACK包丢了，还是数据包被网络延迟了
    
// 滑动窗口机制
    如果发送一段数据需要等到ACK才能发送下一段数据的话效率太低，所以采取批量发送数据的办法，但是发送方也不能随心所欲发送数据啊，所以为了解决这种现象，TCP 提供⼀种机制可以让「发送⽅」根据「接收⽅」的实际接收能⼒控制发送的数据量，这就是滑动窗⼝的由来。
    带宽：单位时间的流量，单位是 Mb/s
    带宽时延积 = 带宽 X RTT ，表示在网络中的数据大小
...
// 拥塞控制机制
    ...
    目的是避免让发送方的数据填满整个网络。
    只要发生重传机制(包括超时重传和快速重传)网路就会认为出现了拥塞。
    swnd = min(cwnd,rwnd)
    
    // (3)(4)是发生重传时候的算法
    算法：
    	(1) 慢启动：当发送⽅每收到⼀个 ACK，拥塞窗⼝ cwnd 的⼤⼩就会加 1。
    	(2) 拥塞避免：到达慢启动门限ssthresh(slow start threshold)时，每当收到⼀个 ACK 时，cwnd 增加 1/cwnd
    	// 网络速度会急剧下降
    	(3) 拥塞发生算法：针对超时重传，cwnd = 1,ssthresh = cwnd/2,接着执行慢启动算法
    	// 由于快速重传只是丢了一小段数据，所以只需要减少一点拥塞窗口cwnd即可
    	(4) 快速恢复算法：cwnd = cwnd/2,ssthresh = cwnd,(cwnd = ssthresh + 3)，cwnd是否加3都行，加是因为三个ACK包收到了，没有在网络中发生拥塞，接着采用拥塞避免算法
...
2.有哪些路由协议
3.域名解析过程，如何干预域名解析
4.DNS查询服务器的基本流程，什么是DNS劫持
5.网关的作用是什么，同一网段的主机如何通信
6.CSRF
7.MAC地址和IP地址
```
...
// 新机安装mysql
    ...
    netstat -anp | grep 3306 // 查看3306的进程
    mysqladmin -u root password  dy19980314
    mysql -uroot -p // 连接数据库
    
// 云端部署步骤
    // 打包上传 项目路径：/var/www/miaosha
    1.打包项目为jar包(针对springboot项目)
    // 外挂配置文件
    2.可以在服务器的部署目录配置一个额外的配置文件，这个配置文件的相同配置可以覆盖项目下的配置
    3.编写一个deploy脚本 (deploy.sh)
    ...
    4.运行：./deploy.sh &
    5.终端输出命令打印在项目目录下的nohup.out中
      用 tail -200f nohup.out查看
     
// jmeter性能压测    
    (1) 首先建立一个线程组，确定发多少个线程请求
    (2) 线程组下建立http请求，确认ip、端口、路径参数等
    (3) 线程组下添加结果树，看每个http请求的响应     (4) 线程组下添加聚合报告。看整体请求的响应时间、qps等 
       
// 问题1：server端并发请求数上不去
    springboot默认内嵌的tomcat配置的问题
      	server.tomcat.accept-count 等待队列：100
        server.tomcat.max-threads 最大工作线程：200
        server.tomcat.min-spare-threads 最小工作线程：10
        server.tomcat.max-connections 最大可连接数：10000
    也就是说在默认配置下，某一时间段触发请求超过200+100就会被拒绝处理，连接超过10000就会被拒绝连接
// 解决办法
    在服务器项目的目录下的挂载配置文件中配置参数
    server.tomcat.accept-count=1000
	server.tomcat.max-threads=800
	server.tomcat.min-spare-threads=100 
    然后查看进程的PID，kill掉，并重启项目
 
// 定制化内嵌的tomcat开发
    主要修改keepalive的参数
        keepalive用于和前面的nginx代理之间保持长链接关系 避免每次被代理后都需要新建一个链接 损伤性能
  
// 问题2：响应时间变长TPS上不去        

// 远端访问mysql需要修改数据库服务器的访问权限
    (1) grant all privileges on *.* to root@'%' identified by 'root' 
    (2) flush privileges // 不用重启数据库就可以刷新配置的权限
nohup // 命令行端口启动一个shell命令，应用程序退出也会在后台运行
        
// openresty        
   路径：/usr/local/openresty 
	sbin/nginx -s reload // 平滑重启
// 分布式配置优化
    分布式配置后可以提高吞吐量，减少Cpu负载，但是会产生网络通信的问题，网络通信问题是由druid连接池、客户端和nginx的长连接、nginx和服务的长连接解决的(默认是短连接)
    // 针对的是查询某一商品的信息，项目中包括商品表，库存表，以及该商品对应的活动表，压测是在1000线程数，20次，5秒内做的
    优化提高：
     	(1) 单机tps在1500左右,延迟的话是500
        (2) 分布式后tps在1700，说的都是峰值，延迟也是500左右，没有太明显的变化，而设置nginx和服务的长连接后，延迟会减少到400。(这里面带宽是很重要的影响因素，设置数据库和nginx服务器的带宽为25M和20M)
        (3) 配置查询的redis缓存后，tps在2000，延迟在250左右
        (4) 用guava做本地缓存(缓存在服务的JVM中)后，tps在3400，延迟在50
        (5) 用nginx做缓存，nginx proxy cache效果会变低
        	用openresty的shared dic 做缓存，所有的worker进程可见，tps在3700，延迟160，相比文件系统的缓存性能提升很多。// 和guava的本地缓存差不多
        	openresty对redis的支持，tps也在3000多，延迟也都差不多，好处在于方便实现数据的更新（因为每次请求都要从远端的redis中读取数据，读不到才访问后面的项目服务器）
// nginx性能高的原因？？？？
    (1) epoll多路复用
    (2) master-worker进程 // work进程是master的子进程
        平滑重启的时候，会将worker进程内的描述符归还给master进程，然后master销毁原worker并重新创建新的worker，将原描述符分给新的worker，完成平滑重启。
    (3) 协程机制
        java的servlet中，每个请求对应一个线程，当发生io阻塞时，线程的切换需要操作系统调度，用户态到内核态及CPU上下文的切换；而nginx的一个请求对应一个worker进程中的协程，当io阻塞时，协程主动放弃执行权，并把阻塞的事件注册到内核中的epoll结构里并注册回调函数，当事件准备就绪时，自动调用回调函数，继续执行原协程的操作。
// 设计缓存的思路
    将缓存推到离用户最近的位置
    脏缓存的清理
...
// springboot 的redis
    会在RedisAutoConfiguration中配置相关redis的信息
        
// 交易----mq的改进
    最开始的代码，去redis中减完库存，然后异步的发送消息，可能会有问题：如果订单创建失败，就会导致redis的库存回滚，但是异步消息已经发送出去，mysql中库存减少，会导致少卖；而第一次修改后，将异步消息在事务最后提交之前发送，有可能在事务提交的时候失败，而消息已经发送，所以想可以否在事务提交后发送消息；而第二次修改后，由Spring提供的TransactionSynchronizationManager将异步消息的发送改为事务提交后，但是消息依然可能发送失败，而减库存和创建订单的事务已经提交，此时redis的减库存操作无法回滚，会导致超卖（就是说mysql中库存没少，而redis少了，订单已创建，会导致超卖）；所以采用rocketmq的事务型消息，使用rocketmq提供的TransactionMQProducer发送消息，会将消息状态设置为两阶段，刚提交到broker为prepare，此时不会被消费，而此时会回调注册在TransactionMQProducer中TransactionListener的executeLocalTransaction方法，在这个方法执行创建订单的逻辑，返回commit状态的话，消息就会被消费，返回rollback就不会被消费。如果executeLocalTransaction方法长时间不返回的话（也就是创建订单的逻辑不返回，有可能init、失败或者成功），mq就会回调checkLocalTransaction方法进行判断，此时就需要库存流水状态的表信息，进行判断了，在下单之前就需要初始化库存流水状态信息数据，最后在初始化库存流水之前，还需要加上是否售罄的判断，因为如果不加，那么在没有库存的时候，也会存入流水信息，这样会冗余
        
// 下单接口的缺陷
    // 一个令牌就可以解决下面问题了吗？
    下单接口被不停的刷，即使活动没开始也会对服务器造成压力
    验证逻辑和下单接口的逻辑耦合度太高
        
// SSO单点登录的问题
    同域名：会带着cookie去SSO中找登录信息
    根域名相同(一级域名)但子域名不同(二级域名)：服务器设置domain = / ,这样客户端下次访问不同的域名也会呆着同一个cookie了
    不同域名：// 课程里有讲解但是还没有仔细研究
// mysql的性能优化
    通用性能优化：缓存、异步、批处理
    应用层面：
        写操作：update、insert用批处理的方式
        	优势：1) sql预编译的时间和空间复杂度降低 2) 网络消耗的时间减少
        读操作：用索引
...
netstat -anp | PID // 查看进程监听的端口
pstree -p 23572 | wc -l // 查看进程有多少个线程
pstree -p 23572 // 查看进程的线程有哪些
top -H // 查看电脑资源使用情况，包括cpu，线程等
yum install lrzsz // 传输文件用的
...
// 查询优化
// 缓存校验 + 异步下单
1.nginx做反向代理，实现解耦，降低CPU的负载，提高吞吐量 // 但延迟也会提高,是由druid连接池、客户端和nginx的长连接、nginx和服务的长连接解决的(默认是短连接)
    
2.分布式下，使用redis + token的方式解决单点tomcat服务器只有一个session的问题
    
3.查询优化
    // 多级缓存的思路
    (1) 在数据库服务器上设置redis缓存，
    (2) 在应用服务器上用guava cache实现本地热点缓存，注意这里是 1)热点缓存，2) 同时这个缓存应该是对脏读不敏感，只是在短时间内有效，生命周期应该比redis的短很多，这样在超过缓存生命周期内就会被guava的机制删除；如果发生变化，由于应用服务器太多，想要改变热点服务的缓存数据可以通过mq通知机制重新获取，但这样会增大网络开销，以及其他的问题;3) 这个缓存空间需要内存可控。综上用guava可以控制缓存的生命周期、大小(lru淘汰)、已经并发读写能力，而hashmap不具备这样的能力。
    (3) 使用nginx的缓存
        1) 用nginx proxy cache，使用uri作为key，value为存储在文件系统中的内容的地址，由于需要去磁盘中找缓存内容，所以性能并没有得到优化，也不会采取这种方案。
        2) 用openresty对Shared dic的支持，用内存做缓存，解决了1) 中tps性能下降的问题。
        3) 用openresty对redis的支持，nginx 对 redis 只读不写，这样对应的数据更新的问题就解决了，更新操作交给下游服务器去做，nginx自己只用关心redis有没有这个数据。可以做到时时感知redis内的数据
        // 总结：guava cache和shared dic对热点数据的缓存都可以提高性能，但问题就是更新数据很麻烦；而针对数据不是特别的热而且还需要更新的时候，建议使用openresty对redis的支持
...
4.交易优化
    问题所在：
    	...
    	2) 库存行锁
    	3) 后续生成订单过程
    (1) 无优化的时候，200个线程，20次，tps200多，延迟在500，1000个线程，20次，tps400多，延迟2000(接近2秒)
    (2) 针对问题 1) 将用户和商品模型的验证内容存入redis，接着以后每次验证的时候就会从redis验证，而不是mysql。1000个线程，tps1200，延迟600
    (3) 针对更新同一item的库存导致的行锁，将库存更新引入redis，然后异步化消息给数据库。
    
5.下单的流量削峰
    
    // 代码步骤逻辑：生成token的controller(控制大闸流量) -> 生成订单controller -> 用线程池进行队列泄洪
    令牌的作用？？？
    使用秒杀令牌实现大闸功能
    根据库存的初始数量确定秒杀令牌的总个数，控制大闸流量
    令牌 = 秒杀活动id + 用户id + 商品id
...
6.下单的限流防刷
    ...
    (2) 限流的策略
    	限制并发：限制一共多少人同时干活
    	令牌桶：限制一秒中可以有几个人干活，限制某一秒流量的最大值，可以应对突发流量，但不能超过限定值。
    	漏桶算法：限制一秒中可以有几个人干活，漏桶算法主要是平滑网络流量，以固定的速率流入的操作。

// 令牌、大闸、队列泄洪
// 验证码：错峰
// 限流：google的rateLimiter
...
个人感觉redis集群目前很多遇到的热key、大key问题都是网络带宽的瓶颈，redis本身处理能力个人感觉没啥大问题； 遇到瓶颈那就少查或者不查，缓存前置到本地缓存、nginx和CDN等
...
3.
4.订单号如何生成，已经分布式id
5.redis的模式、redis缓存失效怎么办，redis的脏读问题？
6.nginx性能高的原理？
7.redis缓存除了课程中使用的简单的key，value结构，其它的数据结构各自适用什么样的场景
    zset用来做排行榜
	bitmap用来做布隆过滤器
	geo用来做谁在附近的功能
    hyperlog list等都可以去看下
```

##### **遇到的问题**

```java
1.redis远程无法连接的问题，具体看CSDN的博客
  redisTemplate一直是null的原因：设置为static字段，类加载的时候就去找springboot的初始对象了，那肯定是找不到的，这时候main方法还没有执行呢
    
2.rocketmq连接超时，最后发现是因为服务器的网络安全组端口没有开放
    
3，服务器起太多，数据库的ip地址弄错了，导致一直请求不到数据，因为把数据库安装到别的ip上了
```
