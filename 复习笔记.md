...
1.如何判断堆中对象可回收
    ...
    (3) 引用的类型
		强引用
		软引用(高速缓存：使用缓存的同时，不会耗尽内存)只被软引用指向的对象当内存快溢出的时候才会被清理掉
...
3.类加载器
    ...
    双亲委派机制的代码loadclass、findclass、defineclass
...
System.gc()为什么不能立即生效
方法调用的指令
invokevirtual 调用对象的实例方法，根据对象的实例类型进行分派(动态分派)
invokespecial 调用一些需要特殊处理的方法,如<init> 私有方法父类方法(都不能重写的方法,静态分派)
